...
class PurchaseOrder(models.Model):
    ...
    taxes = fields.One2many(comodel_name='purchase.order.tax',
                            inverse_name='purchase_order', string='Taxes')
    ...
    def _calc_taxes(self):
        tax_model = self.env['purchase.order.tax']
        for order in self:
            order.taxes.unlink()
            for tax in self.compute(order).values():
                tax_model.create({
                    'purchase_order': tax['order'],
                    'sequence': tax['sequence'],
                    'name': tax['name'],
                    'base': tax['base'],
                    'amount': tax['amount']})
    ...
    def action_wait(self):
        self._calc_taxes()
        return super(PurchaseOrder, self).action_wait()
